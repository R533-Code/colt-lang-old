...
namespace clt::meta
{
  ...
  concept Allocator =
      (std::same_as<std::decay_t<decltype(T::alignment)>, u64>)&&requires(
          T t, mem::MemBlock b, ByteSize<Byte> sz) {
        {
          t.alloc(sz)
  ...
  concept ExpandingAllocator =
      Allocator<T> && requires(T t, mem::MemBlock b, ByteSize<Byte> delta) {
        {
          t.expand(b, delta)
        } -> std::same_as<bool>;
      };
