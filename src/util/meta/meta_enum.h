...
#define COLT_DETAILS_STRINGIZE_ENUM(en) , std::string_view{ #en }
#define COLT_DETAILS_MAP_PAIR_ENUM(en)  , std::pair{ std::string_view{ #en }, en }
...
#define ADD_REFLECTION_FOR_CONSECUTIVE_ENUM(namespace_name, name, first, ...) \
  ...
  struct clt::reflect<namespace_name::name> { \
    ...
    static constexpr std::string_view str() noexcept { return #name; } \
    static constexpr bool is_consecutive() noexcept { return true; } \
    static constexpr std::array name##_str = { std::string_view{ #first} COLT_FOR_EACH(COLT_DETAILS_STRINGIZE_ENUM, __VA_ARGS__) }; \
  ...
  private: \
    using ArrayTable_t = std::array<std::pair<std::string_view, namespace_name::name>, name##_str.size()>; \
    using Map_t = clt::meta::ConstexprMap<std::string_view, namespace_name::name, name##_str.size()>; \
    static constexpr ArrayTable_t get_array() { \
        using enum namespace_name::name; \
        ArrayTable_t ret = { \
          std::pair{ std::string_view{ #first}, first } \
...
ADD_REFLECTION_FOR_CONSECUTIVE_ENUM(clt::meta, EntityKind,
