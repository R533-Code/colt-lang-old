...
namespace clt::lng
{
	...
	*/
	...
	struct Lexer
	{
		...
		{
			return StringView{ buffer.lines[snap.line_nb].data() + snap.column_nb, offset() - snap.column_nb };
		}
		...
		{
			return SourceInfo{ snap.line_nb + 1, StringView{ &buffer.lines[snap.line_nb].front() + snap.column_nb, size_lexeme }, buffer.lines[snap.line_nb] };
		}
		...
		void add_identifier(StringView identifier, const Snapshot& snap) const noexcept
		{
			...
			buffer.add_identifier(identifier, Lexeme::TKN_IDENTIFIER, snap.line_nb, snap.column_nb, size_lexeme);
		}
	...
	void handle_int_with_extension(Lexer& lexer, const Lexer::Snapshot& snap, int base = 10) noexcept;
	...
	*/
	...
	void handle_int_with_extension(Lexer& lexer, const Lexer::Snapshot& snap, int base) noexcept
	{
