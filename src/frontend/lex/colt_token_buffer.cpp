...
namespace clt::lng
{
      ...
      TokenBuffer& buffer, ErrorReporter& reporter, StringView to_parse) noexcept
  {
    ...
    else
    {
      ...
      buffer.add_token(
          Lexeme::TKN_EOF, buffer.line_nb(token) - 1, buffer.column_nb(token) + 1,
          0);
    }
  }
  ...
  void create_lines(StringView strv, TokenBuffer& buffer) noexcept
  {
    ...
    i64 start              = 0;
    while (auto nl =
               static_cast<const char*>(memchr(&text[start], '\n', size - start)))
    {
      i64 nl_index = nl - text;
      // + 1 to include the '\n'
      buffer.add_line(
          StringView{front + start, static_cast<size_t>(nl_index - start) + 1});
      start = nl_index + 1;
    }
